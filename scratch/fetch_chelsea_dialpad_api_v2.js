...
function loadEnvToken() {
  ...
  const match = content.match(/DIALPAD_TOKEN_2\s*=\s*["']?([^"'\r\n]+)/);
  if (!match) {
    throw new Error('DIALPAD_TOKEN_2 not found in .env.local.');
  }
  return match[1];
}
...
async function main() {
  ...
  console.log('Successfully loaded Dialpad Token Slot 2. Fetching calls directly from Dialpad API...');
