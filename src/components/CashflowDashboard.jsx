...
export default function CashflowDashboard({
  ...
  const startingCashTotals = useMemo(() => {
    const totals = {};
    companies.forEach(c => {
      // Filter bank accounts by company selection
      if (selectedCompanyId !== 'all' && c.id !== selectedCompanyId) return;

      (c.bankAccounts || []).forEach(acc => {
        // Filter by bank account selection
  ...
  const inflows = useMemo(() => {
    return placements.map(p => {
      ...
      // Filter by bank account (company of bank account)
  ...
  const outflows = useMemo(() => {
    ...
    // C. Unreconciled Expenses (Outflows)
    const expenseOutflows = expenses.filter(exp => {
      // Pending/unreconciled bills or future scheduled expenses
    ...
    return [...contractOutflows, ...payrollOutflows, ...expenseOutflows].filter(out => {
      ...
      if (selectedBankAccountInfo) {
        // If tied explicitly to a bank account, must match
        if (out.bankAccountId) {
          if (out.bankAccountId !== selectedBankAccountId) return false;
        } else {
          // If not explicitly set, filter by company matching the bank account
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {/* -------------------------------------------------------------
          COMPANY BANKING LIVE BALANCES PANEL
          ------------------------------------------------------------- */}
      <div className="detail-section" style={{ padding: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
          <Wallet size={20} style={{ color: 'var(--primary)' }} />
          <h3 style={{ margin: 0, fontSize: '15px', fontWeight: 800, color: '#fff' }}>Company Banking Portals & Live Cash Balances</h3>
        </div>
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px dashed var(--border-color)', paddingBottom: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <strong style={{ fontSize: '13px', color: '#fff' }}>{comp.name}</strong>
                  </div>
                    ...
                    return (
                      ...
                      >
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                          <span style={{ fontSize: '11px', fontWeight: 700, color: '#fff' }}>{acc.bankName}</span>
                          <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{acc.accountName} ({acc.accountNumber})</span>
                        </div>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setSelectedBankAccountId('all'); // reset bank account when changing company
            }}
      ...
      {/* -------------------------------------------------------------
          KPI METRICS TILES (With starting cash and projected cash)
          ------------------------------------------------------------- */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1.2fr', gap: '16px' }}>
        ...
        <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '12px', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <h2 style={{ fontSize: '18px', fontWeight: 800, margin: '4px 0 2px 0', color: '#fff', fontFamily: 'monospace' }}>
              {startingCashTotals}
            </h2>
        ...
        <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--primary)', borderRadius: '12px', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <h2 style={{ fontSize: '18px', fontWeight: 800, margin: '4px 0 2px 0', fontFamily: 'monospace', color: '#fff' }}>
