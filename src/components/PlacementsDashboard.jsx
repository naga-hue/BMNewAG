    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {activeSubTab === 'matrix' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h2 style={{ fontSize: '18px', fontWeight: 600 }}>YTD Client Placements & Billing Matrix</h2>
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>Track billing distribution and placement performance across client companies by month.</p>
            </div>
          ...
          <div className="table-container" style={{ overflowX: 'auto', width: '100%' }}>
            <table className="entity-table dense" style={{ minWidth: '1300px', fontSize: '12px' }}>
              <thead>
                <tr>
                  <th style={{ minWidth: '180px' }}>Client Company</th>
              ...
              <tbody>
                {(() => {
                  const uniqueClients = Array.from(new Set(placements.map(p => p.clientCompany).filter(Boolean))).sort();
                    ...
                    return (
                      <tr>
                        <td colSpan="14" style={{ textAlign: 'center', padding: '24px', color: 'var(--text-secondary)' }}>
                          No placements recorded in database.
                        </td>
                      </tr>
                    );
                  }
                          ...
                          <tr key={clientName}>
                            ...
                            {[...Array(12)].map((_, monthIdx) => {
                              const cellPlacements = placements.filter(p => {
                                ...
                                if (p.clientCompany !== clientName) return false;
                                const pStart = new Date(p.startDate);
                                return pStart.getFullYear() === Number(matrixYear) && pStart.getMonth() === monthIdx;
                              });

                              const cellSum = cellPlacements.reduce((sum, p) => sum + (Number(p.netScoreValue) || 0), 0);
                              rowTotal += cellSum;
                              colTotals[monthIdx] += cellSum;
                              grandTotal += cellSum;
                              ...
                              return (
                                <td key={monthIdx} style={{ textAlign: 'right' }}>
                                  {cellSum > 0 ? (
                                    ...
                                    >
                                      £{cellSum.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                                    </button>
                                  ) : (
                            ...
                            <td style={{ textAlign: 'right', fontWeight: 700, backgroundColor: 'rgba(255,255,255,0.01)', color: 'var(--accent)' }}>
                      ...
                      <tr style={{ fontWeight: 700, backgroundColor: 'rgba(255,255,255,0.03)', borderTop: '2px solid var(--border-color)' }}>
                        ...
                        {colTotals.map((tot, idx) => (
                          <td key={idx} style={{ textAlign: 'right', color: 'var(--success)' }}>
                            £{tot.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                          </td>
                        ))}
                  ...
                  <table className="entity-table dense" style={{ fontSize: '11px' }}>
                    <thead>
                      <tr>
                        ...
                        <th>Start Date</th>
                        <th style={{ textAlign: 'right' }}>Net Score</th>
                        <th>Source</th>
                    ...
                    <tbody>
                      {selectedCellPlacements.map(p => (
                        <tr key={p.id}>
                          ...
                          <td>{p.startDate}</td>
                          <td style={{ textAlign: 'right', fontWeight: 700, color: 'var(--success)' }}>
                            £{p.netScoreValue.toLocaleString()}
                          </td>
                          <td>{p.source}</td>
                          <td>
                            {p.splits?.map(s => {
                              ...
                              return (
                                <div key={s.staffId} style={{ fontSize: '10px' }}>
                                  {r ? r.fullName : 'Recruiter'}: {s.percentage}%
                                </div>
                              );
                            })}
                          </td>
